...
// bool PCI::next_device_info(DeviceInfoIterator &it, Identification &out_ident, ClassInfo &out_cls)
// {
//     // Начинаем сканирование с текущей позиции
//     while(it.addr.bus < 256) 
//     {
//         while(it.addr.device < 32) 
//         {
//             while(it.addr.function < 8) 
//             {
//                 // Проверяем текущую функцию
//                 it.addr.register_offset = Identification::RegisterID;
//                 out_ident.raw = device_get(it.addr);
...
//                     // Подготавливаем следующую позицию для следующего вызова
//                     ++it.addr.function;
//                     if(it.addr.function >= 8) 
//                     {
//                         it.addr.function = 0;
//                         ++it.addr.device;
//                         if(it.addr.device >= 32) 
//                         {
//                             it.addr.device = 0;
//                             ++it.addr.bus;
//                         }
...
//                 // Устройства нет, переходим к следующей функции
//                 ++it.addr.function;
//             }
            
//             // Все функции проверены, переходим к следующему устройству
//             it.addr.function = 0;
//             ++it.addr.device;
//         }
        
//         // Все устройства проверены, переходим к следующей шине
//         it.addr.device = 0;
//         ++it.addr.bus;
//     }
